...
import { getCurrentDayNumber, getPDTTime } from "../utils/time-utils";
...
export class ReminderService {
	static async sendGoalReminders(
		...
		try {
			...
			for (const member of challengeMembers.values()) {
				const isCompleted =
					await ReminderService.checkUserCompletionStatus(member);
				if (!isCompleted) {
	...
	static scheduleGoalReminders(): void {
		...
		ReminderService.scheduleReminder(
			REMINDER_TIMES.REGULAR_REMINDER.hour,
			REMINDER_TIMES.REGULAR_REMINDER.minute,
			false,
		);
		ReminderService.scheduleReminder(
			REMINDER_TIMES.LAST_CALL_REMINDER.hour,
			REMINDER_TIMES.LAST_CALL_REMINDER.minute,
			true,
		);
	}
	...
	private static scheduleReminder(
		...
		setTimeout(async () => {
			try {
				...
				if (guild) {
					await ReminderService.sendGoalReminders(guild, isLastCall);
				}
			} catch (error) {
				handleError(
					error,
					`sending ${isLastCall ? "last call" : "reminder"} reminder`,
				);
			}

			ReminderService.scheduleReminder(hour, minute, isLastCall);
		}, timeUntilReminder);
