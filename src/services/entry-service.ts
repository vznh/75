...
import { EntrySession, GoalSubmission } from '../models/entry';
import { validateSubmission, getValidationRequirements } from '../utils/validation-utils';
import { handleError, handleAsyncError } from '../middleware/error-handler';
import { logInfo, logError, logDebug } from '../middleware/logging';
import { isLastDayOfWeek, getCurrentDayNumber } from '../utils/time-utils';
...
export class EntryService {
      ...
      if (shouldShowWeeklyGoals && userWeeklyGoals.length > 0) {
        for (const weeklyGoalDef of userWeeklyGoals) {
          try {
            ...
            handleError(error, `sending weekly goal embed for ${weeklyGoalDef.name}`);
          }
        }
      }
    } catch (error) {
  ...
  static async showPreviousEntries(member: GuildMember): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        logInfo(`Sent previous entries embed to user ${member.displayName || member.user.username} via DM`);
      } catch (dmError) {
  ...
  static async shareThread(threadId: string, userId: string): Promise<void> {
    try {
      ...
      const shareChannel = client.channels.cache.get(CHANNEL_IDS.SHARE) as TextChannel;
  ...
  private static async sendInitialEntryEmbed(thread: ThreadChannel, displayName: string, dayNumber: number, userGoals: any[], userWeeklyGoals: any[]): Promise<void> {
    try {
      let description = `\n\n(￣✓￣) ☑ . . . . your goals\n`;
  ...
  private static async sendGoalEmbed(thread: ThreadChannel, goalDef: any): Promise<Message> {
    try {
      const embed = new EmbedBuilder()
  ...
  private static async sendWeeklyGoalEmbed(thread: ThreadChannel, goalDef: any, dayNumber: number): Promise<Message> {
    try {
      const embed = new EmbedBuilder()
  ...
  private static async parseAndValidateSubmission(message: Message, session: EntrySession): Promise<void> {
    try {
      ...
      try {
        await channel.send(`✅ **${goalDef.name}** ${MESSAGES.SUBMISSION.SUBMISSION_SUCCESS}`);
      } catch (error) {
    ...
    try {
      ...
      if (thread) {
        try {
          ...
          logInfo(`Thread ${thread.name} is already in archive channel ${thread.parentId}`);
  ...
  private static async updateStatusWithRetry(guild: Guild, displayName: string, maxRetries: number): Promise<void> {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        logInfo(`Attempting status update (attempt ${attempt}/${maxRetries}) for ${displayName}`);
        ...
        if (isCompleted) {
          ...
          logInfo(`✅ Status updated successfully for ${displayName} (attempt ${attempt})`);
          return;
        } else {
          logInfo(`❌ User ${displayName} not showing as completed yet (attempt ${attempt})`);
          if (attempt < maxRetries) {
            await new Promise(resolve => setTimeout(resolve, 1000 * attempt));
          }
        }
      } catch (error) {
        handleError(error, `updating status for ${displayName} (attempt ${attempt})`);
        if (attempt < maxRetries) {
          await new Promise(resolve => setTimeout(resolve, 1000 * attempt));
        }
      }
    }
    logError(`Failed to update status for ${displayName} after ${maxRetries} attempts`);
  }

  private static async updateHistoryWithRetry(guild: Guild, displayName: string, maxRetries: number): Promise<void> {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        logInfo(`Attempting history update (attempt ${attempt}/${maxRetries}) for ${displayName}`);
        
        const { StatusService } = await import('./status-service');
        await StatusService.updateHistoryEmbed(guild);
        logInfo(`✅ History updated successfully for ${displayName} (attempt ${attempt})`);
        return;
      } catch (error) {
        handleError(error, `updating history for ${displayName} (attempt ${attempt})`);
        if (attempt < maxRetries) {
          await new Promise(resolve => setTimeout(resolve, 1000 * attempt));
        }
      }
    }
    logError(`Failed to update history for ${displayName} after ${maxRetries} attempts`);
  }
