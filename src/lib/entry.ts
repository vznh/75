...
import type { EntrySession, GoalDefinition, GoalSubmission } from "../types";
import {
...
export class EntryService {
	static getCurrentDayNumber(): number {
		...
		console.log(
			`Date calculation: Start=${startDate.toISOString()}, Now=${pdtNow.toISOString()}, DiffDays=${diffDays}`,
		);
	...
	static async startEntry(member: GuildMember): Promise<void> {
		try {
			...
			if (existingThread) {
				console.log(
					`Thread already exists for ${displayName} - Day ${dayNumber}`,
				);
			...
			if (shouldShowWeeklyGoals && userWeeklyGoals.length > 0) {
				for (const weeklyGoalDef of userWeeklyGoals) {
					try {
						...
						console.error(
							`Error sending weekly goal embed for ${weeklyGoalDef.name}:`,
							error,
						);
					}
				}
	...
	private static async sendInitialEntryEmbed(
		...
		displayName: string,
		dayNumber: number,
		userGoals: GoalDefinition[],
		userWeeklyGoals: GoalDefinition[],
	): Promise<void> {
		try {
			let description = `\n\n(￣✓￣) ☑ . . . . your goals\n`;
	...
	static async createRoleSelectionMessage(channelId: string): Promise<void> {
		try {
			...
			try {
				...
				console.error(
					"Error checking/deleting existing role selection message:",
					error,
				);
			...
			let description =
				"React with the emoji to add the goal to your entries.\n\n";
			...
			for (const goal of allGoals) {
				try {
					...
					console.error(
						`Error adding reaction ${goal.emoji} for ${goal.name}:`,
						error,
					);
				}
			}
			...
			if (hasRole) {
				...
				try {
					...
					if (role) {
						...
						try {
							await user.send(
								`❌ Removed **${goalDef.name}** goal from your roles.`,
							);
						} catch (error) {
	...
	static async checkUserCompletionStatus(
		member: GuildMember,
	): Promise<boolean> {
		try {
			const archiveChannelId = "1424342473198796961";
			...
			try {
				...
				console.error(
					"Error refreshing thread cache for completion check:",
					error,
				);
	...
	static async updateAccountabilityStatus(guild: Guild): Promise<void> {
		try {
			const channel = (await guild.channels.fetch(
				EntryService.STATUS_CHANNEL_ID,
			)) as TextChannel;
			...
			for (const member of challengeMembers.values()) {
				const completed = await EntryService.checkUserCompletionStatus(member);
			...
			if (EntryService.statusMessageId) {
				try {
					const message = await channel.messages.fetch(
						EntryService.statusMessageId,
					);
					await message.edit({ embeds: [embed] });
					console.log("Updated accountability status embed");
				} catch (error) {
					console.error(
						"Error updating status message, will create new one:",
						error,
					);
					EntryService.statusMessageId = null;
				}
			}

			if (!EntryService.statusMessageId) {
				const message = await channel.send({ embeds: [embed] });
				EntryService.statusMessageId = message.id;
	...
	static async resetDailyStatus(): Promise<void> {
		...
		try {
			...
			const client = (await import("../index")).client;
			await EntryService.updateAccountabilityStatus(
				...
				try {
					...
					if (!role) {
						...
						role = await guild.roles.create({
							name: roleName,
							color: EntryService.getGoalColor(roleName),
					...
					if (role.color !== EntryService.getGoalColor(roleName)) {
						await role.edit({
							color: EntryService.getGoalColor(roleName),
			...
			for (const goalDef of allWeeklyGoals) {
				...
				try {
					...
					if (!role) {
						// Create the role if it doesn't exist
						console.log(
							`Creating weekly role: ${roleName} with emoji: ${emoji}`,
						);
	...
	private static async sendWeeklyGoalEmbed(
		...
		try {
			...
			console.error(
				`Error sending weekly goal embed for ${goalDef.name}:`,
				error,
			);
			throw error;
		}
	...
	static async updateHistoryEmbed(guild: Guild): Promise<void> {
		try {
			...
			const historyData = await EntryService.reconstructHistoryFromThreads(
				guild,
				challengeMembers,
			);
			...
			const embed = new EmbedBuilder()
				.setTitle("History")
				.setDescription(EntryService.formatHistoryData(historyData))
				.setColor(0x0099ff)
				.setTimestamp();

			// Update or send message
			if (EntryService.historyMessageId) {
				try {
					const message = await channel.messages.fetch(
						EntryService.historyMessageId,
					);
					await message.edit({ embeds: [embed] });
					console.log("Updated history embed");
				} catch (error) {
					console.error(
						"Error updating history message, will create new one:",
						error,
					);
					EntryService.historyMessageId = null;
				}
			}

			if (!EntryService.historyMessageId) {
				const message = await channel.send({ embeds: [embed] });
				EntryService.historyMessageId = message.id;
	...
	private static async reconstructHistoryFromThreads(
		...
		try {
			...
			const currentDay = EntryService.getCurrentDayNumber();
	...
	private static async updateStatusWithRetry(
		...
		for (let attempt = 1; attempt <= maxRetries; attempt++) {
			try {
				...
				if (isCompleted) {
					await EntryService.updateAccountabilityStatus(guild);
					console.log(
				...
				console.error(
					`Error updating status for ${displayName} (attempt ${attempt}):`,
					error,
				);
				if (attempt < maxRetries) {
					await new Promise((resolve) => setTimeout(resolve, 1000 * attempt));
	...
	private static async updateHistoryWithRetry(
		...
		for (let attempt = 1; attempt <= maxRetries; attempt++) {
			try {
				...
				await EntryService.updateHistoryEmbed(guild);
				console.log(
				...
				console.error(
					`Error updating history for ${displayName} (attempt ${attempt}):`,
					error,
				);
				if (attempt < maxRetries) {
					await new Promise((resolve) => setTimeout(resolve, 1000 * attempt));
	...
	static async sendGoalReminders(
		guild: Guild,
		...
		try {
			...
			const individualPings = incompleteMembers
			...
			await channel.send(message);
			console.log(
				`✅ Sent ${isLastCall ? "last call" : "reminder"} message to ${incompleteMembers.length} members`,
			);
		} catch (error) {
	...
	static scheduleGoalReminders(): void {
		...
		EntryService.scheduleReminder(23, 0, false); // 11:00 PM

		// Schedule 11:30 PM reminder
		EntryService.scheduleReminder(23, 30, true); // 11:30 PM (last call)
	}
	...
	static async shareThread(threadId: string, userId: string): Promise<void> {
		try {
			...
			console.log(
				`Successfully shared thread ${threadId} for user ${displayName}`,
			);
		} catch (error) {
	...
	private static scheduleReminder(
		...
		setTimeout(async () => {
			try {
				...
				if (guild) {
					await EntryService.sendGoalReminders(guild, isLastCall);
				}
			} catch (error) {
				console.error(
					`Error sending ${isLastCall ? "last call" : "reminder"} reminder:`,
					error,
				);
			}

			// Schedule next day's reminder
			EntryService.scheduleReminder(hour, minute, isLastCall);
		}, timeUntilReminder);

		const reminderType = isLastCall ? "last call" : "reminder";
		console.log(
			`⏰ Scheduled ${reminderType} for ${targetTime.toLocaleString("en-US", { timeZone: "America/Los_Angeles" })}`,
		);
	}
}
